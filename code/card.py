...
def load_card_old(short_name, player=False):
    import functools
    import sys
    ...
    c.name = data['Name']
    c.long_name = data['LongName'] if data['LongName'] != '' else c.name
    c.color_name = data['Class']
    if c.color_name:
        c.color = Colors[c.color_name]
    c.type = data['Type']
    if data['Cost'] != '':
        c.cost = int(data['Cost'])
        c.starting_cost = int(data['Cost'])
    if data['Attack'] != '':
        c.attack = int(data['Attack'])
        c.starting_attack = int(data['Attack'])
    if data['Health'] != '':
        c.health = int(data['Health'])
        c.starting_health = int(data['Health'])
    if data['Amount'] != '':
        c.amount = int(data['Amount'])
        c.starting_amount = int(data['Amount'])
    if data['Handicap'] != '':
        c.handicap = float(data['Handicap'])
    c.special_text = data['Text']
    c.special = [data['Keywords']]
    if ' ' in c.special[0]:  # Multiple keyword abilities
        c.special = c.special[0].split(' ')
    c.starting_special = list(c.special)
    c.active_locations = [data['ActiveLocations']]  # Abilities active in hand, deck or discard
    if ' ' in c.active_locations[0]: # Multiple active locations
        c.active_locations = c.active_locations[0].split(' ')
    c.active_locations.extend(['board', 'tableau'])
    c.tags = [data['Tags']]
    if ' ' in c.tags[0]:  # Multiple keyword abilities
        c.tags = c.tags[0].split(' ')
    c.tags = [t for t in c.tags if t in tag_file_dict]
    c.starting_tags = list(c.tags)
    c.n_hand_targets = int(data['HandTargets'])
    c.n_board_targets = int(data['BoardTargets'])
    c.filename = data['Filename']
    if data['Include'] == 'Yes':
        c.include = True
    else:
        c.include = False
    c.priority = int(data['Priority'])
    c.ability_cost = int(data['AbilityCost'])
    c.starting_ability_cost = c.ability_cost
